classdef Autoland_drone < handle
    ...
    methods
        function obj = Autoland_drone(startPos, initialYaw, droneDiameter, simDt)
            obj.Position = startPos(:);
            obj.Yaw = initialYaw;
            obj.armLength = droneDiameter * 0.4;
            obj.propRadius = droneDiameter * 0.1;
            obj.dt = simDt;
        ...
        function update(obj, scanPoints, terrainF, landingSites)
            ...
            switch obj.State
                ...
                case 'APPROACH', targetVel = obj.approachBehavior(landingSites, terrainF);
            ...
            if ~isnan(groundH)
                safeAlt = 3;
                if strcmp(obj.State, 'DESCEND'), safeAlt = 0.1; end
        ...
        function targetVel = approachBehavior(obj, landingSites, terrainF)
            ...
            groundZ = terrainF(landingXY(1), landingXY(2));

            horizontalError = landingXY - obj.Position(1:2);
            ...
            % 同时下降高度：距离越近，目标高度越低（斜线下降）
            % 距离8m时目标高度=地面+5m，距离0.5m时目标高度=地面+2m
            targetHeight = groundZ + max(2.0, min(5.0, 2.0 + distXY * 0.4));
            zError = targetHeight - obj.Position(3);
            vz = max(-2.0, min(1.5, zError * 0.8));

            targetVel = [targetVel_XY; vz];
        end
        ...
        function targetVel = descendBehavior(obj, terrainF)
            ...
            landingXY = obj.TargetLandingPos(1:2);
            groundZ = terrainF(landingXY(1), landingXY(2));
            targetZ = groundZ + 0.1;  % 0.1m for landing gear
            ...
            if distXY > 0.5
                ...
                if abs(zError) < 0.3 && distXY <= 0.5 && norm(obj.Velocity) < 1.0
